...
log_writer = SummaryWriter('runs/cws/' + PRETRAIN_MODEL)
...
def set_logger():
    log_file = os.path.join('save/cws/', 'cws_log.txt')
    logging.basicConfig(
...
def main(args):
    ...
    # model = CWS(tag2id, args.embedding_dim, args.hidden_dim, PRETRAIN_PATH + PRETRAIN_MODEL)
    model = torch.load('save/ner/model_epoch19.pkl')
    # model = torch.load('save/cws/model_rb916.pkl')
